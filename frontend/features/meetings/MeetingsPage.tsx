...
export function MeetingsPage({ employee = false }: { employee?: boolean }) {
  ...
  return <>
    ...
    <div className="meeting-layout"><div className="date-poster"><span>NEXT UP</span><strong>{!loading && !error && items[0] ? new Date(items[0].date).getDate() : "--"}</strong><h3>{!loading && !error && items[0] ? new Date(items[0].date).toLocaleDateString("en-US",{month:"long",weekday:"long"}) : "No meetings"}</h3><p>{!loading && !error && items[0] ? `${items[0].time.slice(0,5)} - ${items[0].location}` : "Schedule the next meeting"}</p></div><div className="meeting-stack">{!loading && !error && items.map((m,i)=><article key={m.id}><div className="meeting-number">0{i+1}</div><div><Badge tone={i===0?"Important":"neutral"}>{m.department}</Badge><h2>{m.title}</h2><p>{m.description}</p><span>{new Date(m.date).toLocaleDateString("en-IN",{day:"2-digit",month:"long"})} - {m.time.slice(0,5)} - {m.location}</span></div>{!employee && <button disabled={deletePendingId !== null} onClick={()=>deleteMeeting(m.id)}><Trash2 size={17}/></button>}</article>)}</div></div>
